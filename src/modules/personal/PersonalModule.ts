...
export class PersonalModule extends BaseModule {
  ...
  formatSearchResult(memory: Memory): Memory {
    ...
    if (metadata.emotional_valence !== undefined) {
      const emotion =
        metadata.emotional_valence > 0.5
          ? 'positive'
          : metadata.emotional_valence < -0.5
            ? 'negative'
            : 'neutral';
  ...
  private assessImportance(content: string, metadata: PersonalMetadata): number {
    ...
    const lowerContent = content.toLowerCase();
    const importanceMatches = importanceKeywords.filter(keyword =>
  ...
  private analyzeEmotionalTrends(memories: any[]): any {
    const withValence = memories.filter(
      m => (m.metadata as PersonalMetadata).emotional_valence !== undefined,
    );
    ...
    const totalValence = withValence.reduce(
      (sum, m) => sum + ((m.metadata as PersonalMetadata).emotional_valence || 0),
      0,
    );
    ...
    return {
      ...
      positive_count: withValence.filter(
        m => (m.metadata as PersonalMetadata).emotional_valence! > 0.3,
      ).length,
      negative_count: withValence.filter(
        m => (m.metadata as PersonalMetadata).emotional_valence! < -0.3,
      ).length,
    };
  }
